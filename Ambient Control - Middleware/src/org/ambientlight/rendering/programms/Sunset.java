...
public class Sunset extends RenderingProgramm {
	...
	Color backgroundTopEnd = new Color(5, 0, 20);
	...
	private void renderSun(BufferedImage pixelmap) {
		...
		float radius = pixelmap.getWidth() / 2;
	...
	private void renderBackground(BufferedImage pixelmap) {
		...
		Point center = getPositionOfSun(pixelmap);
		center.setLocation(center.x, (pixelmap.getHeight() / 0.9) + pixelmap.getHeight() * (position) * 1.45);
		Point focus = getPositionOfSun(pixelmap);
		focus.setLocation(center.x, (pixelmap.getHeight() / 0.9) + pixelmap.getHeight() * (position) * 1.25);

		float[] dist = { 0.0f, (float) (0.1f + (0.09 * position)), 0.32f };
		Color[] colors = { backgroundBottom, backgroundBottom, backgroundTop };
		float radius = (float) (pixelmap.getWidth() * 3.5 + pixelmap.getWidth());
		RadialGradientPaint paint = new RadialGradientPaint(center, radius, focus, dist, colors, CycleMethod.NO_CYCLE);
		Graphics2D g = pixelmap.createGraphics();
		g.setPaint(paint);
		g.fillRect(0, 0, pixelmap.getWidth(), pixelmap.getHeight());
