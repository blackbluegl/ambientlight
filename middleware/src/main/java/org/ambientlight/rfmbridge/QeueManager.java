		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				if (entry.state == State.WAITING_FOR_RESPONSE) {
					...
					try {
						messageListeners.get(entry.message.getDispatcherType()).onResponse(entry.state, null, entry.message);
					} catch (Exception e) {
	...
	private MessageEntry getAwaitingRequestMessageForInMessage(Message inMessage, MessageListener listener) {
		...
		for (MessageEntry current : outQueue) {
			if (current.state == State.WAITING_FOR_RESPONSE
					&& ((RequestMessage) current.message).getCorrelation().equals(((ResponseMessage) inMessage).getCorrelator()) == true) {
				result = current;
				break;
			}
		}
