...
static const RF22::ModemConfig MODEM_CONFIG_TABLE[] = { { 0x2b, 0x03, 0xf4, 0x20, 0x41, 0x89, 0x00, 0x36, 0x40, 0x0a, 0x1d, 0x80,
		0x60, 0x10, 0x62, 0x2c, 0x00, 0x08 }, // Unmodulated carrier
...
uint8_t RF22::adcRead(uint8_t adcsel, uint8_t adcref, uint8_t adcgain, uint8_t adcoffs) {
...
// so YMMV
bool RF22::setFrequency(float centre, float afcPullInRange) {
...
// Returns true if centre + (fhch * fhs) is within limits
bool RF22::setFHStepSize(uint8_t fhs) {
...
// Returns true if centre + (fhch * fhs) is within limits
bool RF22::setFHChannel(uint8_t fhch) {
...
bool RF22::fillTxBuf(const uint8_t* data, uint8_t len) {
	clearTxBuf();
	if (!len)
		return false;
...
void RF22::setPromiscuous(bool promiscuous) {
