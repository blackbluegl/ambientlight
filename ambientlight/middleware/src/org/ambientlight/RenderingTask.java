...
class RenderingTask extends TimerTask {
	...
	private void resetConnection() {
		for (DeviceDriver currentDevice : AmbientLight.room.getDevices()) {
			try {
	...
	@Override
	public void run() {
		AmbientLight.renderer.render();
		
		if(AmbientLight.debug){
			handleDebug();
		}
		
		for (DeviceDriver currentDevice : AmbientLight.room.getDevices()) {
			try {
				if(AmbientLight.renderer.hadDirtyRegionInLastRun()){
	...
	private void handleDebug() {
		if(this.debugRoomDisplay == null){
			this.debugRoomDisplay = new BufferedImageDisplayOutput(AmbientLight.room
					.getRoomBitMap().getWidth(), AmbientLight.room.getRoomBitMap().getHeight(),
					"RoomContent");
			}
		
		Graphics2D g2d = AmbientLight.room.getRoomBitMap().createGraphics();
		AlphaComposite alpha = AlphaComposite.getInstance(AlphaComposite.SRC_OVER,0.6f);
		g2d.setComposite(alpha);
		Font font = new Font("Arial", Font.BOLD, 8);
		g2d.setFont(font);
		
		for(StripePart current : AmbientLight.room.getAllStripePartsInRoom()){
		...
		debugRoomDisplay.setImageContent(AmbientLight.room.getRoomBitMap());
	}
}
